...
/**
 * Benchmarks AtomicLong vs LongAdder under high contention.
 * Many threads increment each counter and we compare wall-clock time.
 *
 * Run: javac LongAdderBenchmarkSnippet.java && java LongAdderBenchmarkSnippet
 * (See AtomicCounterDemo.java for a more complete runnable example)
 */
public class LongAdderBenchmarkSnippet {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        long atomicTime = benchmark("AtomicLong", () -> {
            atomicCounter.incrementAndGet();
        });

        // --- LongAdder ---
        LongAdder adderCounter = new LongAdder();
        long adderTime = benchmark("LongAdder ", () -> {
            adderCounter.increment();
        });

        System.out.println();
        System.out.printf("AtomicLong  final value: %,d%n", atomicCounter.get());
        System.out.printf("LongAdder   final value: %,d%n", adderCounter.sum());
        System.out.printf("AtomicLong  time: %,d ms%n", atomicTime);
        System.out.printf("LongAdder   time: %,d ms%n", adderTime);
        System.out.printf("Speedup: %.2fx%n", (double) atomicTime / adderTime);
    }

    static long benchmark(String name, Runnable increment)
        ...
        System.out.printf("[%s] %d threads x %,d increments => %,d ms%n",
                name, THREADS, INCREMENTS, elapsed);
        return elapsed;
    }
}
```
