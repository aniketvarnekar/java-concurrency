...
/**
 * Demonstrates the CAS loop pattern using a lock-free stack built with AtomicReference.
 * Shows how the retry loop works and prints when retries occur.
 *
 * Run: javac CASInternalsSnippet.java && java CASInternalsSnippet
 * (See CASDemo.java for the full runnable example with ABA demonstration)
 */
public class CASInternalsSnippet {
    ...
    static class LockFreeStack<T> {
        ...
        public void push(T value) {
            ...
            do {
                ...
                newNode.next = currentHead; // 2. Point new node at current head
                attempts++;
                // 3. CAS: set head to newNode only if it is still currentHead
            } while (!head.compareAndSet(currentHead, newNode));

            if (attempts > 1) {
                System.out.println(Thread.currentThread().getName()
                        + " push needed " + attempts + " attempts");
            }
        }
        ...
        public T pop() {
            ...
            do {
                ...
                attempts++;
                // CAS: advance head to next only if head hasn't changed
            } while (!head.compareAndSet(currentHead, currentHead.next));

            if (attempts > 1) {
                System.out.println(Thread.currentThread().getName()
                        + " pop needed " + attempts + " attempts");
            }
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        System.out.println("Popped " + count + " items (expected 40)");
    }
}
```
