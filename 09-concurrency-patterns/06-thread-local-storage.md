...
public class ThreadLocalDemo {
    ...
    // ---------------------------------------------------------------
    // Part 1: Per-thread request ID isolation
    // ---------------------------------------------------------------
    static final ThreadLocal<String> CURRENT_USER =
            ThreadLocal.withInitial(() -> null);

    static void handleRequest(int threadNum, int requestNum) {
        String user = "user-" + threadNum + "-request-" + requestNum;
        CURRENT_USER.set(user);
        try {
            // Simulate processing — any code here can call CURRENT_USER.get()
            System.out.printf("[%s] handling request for user: %s%n",
                    Thread.currentThread().getName(), CURRENT_USER.get());
            // Simulate work
            Thread.sleep(10);
            // Confirm identity is still correct after work
            System.out.printf("[%s] completed request for user: %s%n",
                    Thread.currentThread().getName(), CURRENT_USER.get());
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
        } finally {
            CURRENT_USER.remove(); // mandatory: clean up before returning thread to pool
        }
    }

    // ---------------------------------------------------------------
    // Part 2: Memory leak demonstration (without and with remove())
    // ---------------------------------------------------------------
    static final ThreadLocal<byte[]> LARGE_VALUE =
            ThreadLocal.withInitial(() -> null);

    static void demonstrateLeakWithoutRemove(String threadName) {
        // Set a large value — intentionally NOT calling remove()
        LARGE_VALUE.set(new byte[1024 * 1024]); // 1 MB
        System.out.printf("[%s] LEAK DEMO: set large value, NOT removing it%n", threadName);
        // Thread returns to "pool" — large value persists in ThreadLocalMap
        // Next request on this thread will find the stale 1MB value
        String nextGet = (LARGE_VALUE.get() != null) ? "non-null (STALE)" : "null";
        System.out.printf("[%s] LEAK DEMO: on next get(), value is: %s%n", threadName, nextGet);
    }

    static void demonstrateLeakFixed(String threadName) {
        try {
            LARGE_VALUE.set(new byte[1024 * 1024]); // 1 MB
            System.out.printf("[%s] FIXED DEMO: set large value, will remove in finally%n",
                    threadName);
            // simulate work
        } finally {
            LARGE_VALUE.remove(); // value is now eligible for GC
            System.out.printf("[%s] FIXED DEMO: removed; next get() returns null: %b%n",
                    threadName, LARGE_VALUE.get() == null);
        }
    }

    public static void main(String[] args) throws InterruptedException {
        System.out.println("=== Part 1: Per-thread request context isolation ===\n");
        ...
        // Run without remove — show stale value persists
        Thread leakThread = new Thread(() -> {
            String name = Thread.currentThread().getName();
            demonstrateLeakWithoutRemove(name);
            // Simulate second request on the same thread
            System.out.printf("[%s] Second request: LARGE_VALUE is %s (stale from request 1)%n",
                    name, LARGE_VALUE.get() != null ? "still set!" : "null");
        }, "leak-demo-thread");
        leakThread.start();
        leakThread.join();
        ...
        // Run with remove — show clean state
        Thread fixedThread = new Thread(() -> {
            String name = Thread.currentThread().getName();
            demonstrateLeakFixed(name);
            // Simulate second request on the same thread
            System.out.printf("[%s] Second request: LARGE_VALUE is %s (clean)%n",
                    name, LARGE_VALUE.get() == null ? "null" : "non-null");
        }, "fixed-demo-thread");
        fixedThread.start();
        fixedThread.join();
