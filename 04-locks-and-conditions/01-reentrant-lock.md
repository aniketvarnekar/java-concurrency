...
A bank account using `ReentrantLock`, demonstrating the `try/finally` pattern, timed `tryLock` for deadlock-safe transfers, and `lockInterruptibly` for cancellable operations.
...
public class BankAccount {
    ...
    // Standard deposit — uses try/finally to guarantee unlock
    public void deposit(int amount) {
        lock.lock();
        try {
            balance += amount;
    ...
    // Deadlock-safe transfer: acquires both locks with timeout
    public static boolean transfer(BankAccount from, BankAccount to, int amount)
        ...
        try {
            ...
            if (!fromLocked) {
                System.out.println(Thread.currentThread().getName()
                    + ": could not lock " + from.name + " — aborting transfer");
                return false;
            }
            ...
            if (!toLocked) {
                System.out.println(Thread.currentThread().getName()
                    + ": could not lock " + to.name + " — aborting transfer");
                return false;
            }

            if (from.balance < amount) {
                System.out.println(Thread.currentThread().getName()
                    + ": insufficient funds in " + from.name);
                return false;
            }
            ...
            // Always unlock in reverse acquisition order
            if (toLocked)   to.lock.unlock();
            if (fromLocked) from.lock.unlock();
        }
    }

    // Interruptibly wait for the lock — allows cancellation of long waits
    public void interruptibleDeposit(int amount) throws InterruptedException {
        ...
        try {
            balance += amount;
            System.out.println(Thread.currentThread().getName()
                + ": interruptible deposit of " + amount + " to " + name);
        } finally {
            lock.unlock();
        }
    }
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        // Demo 1: concurrent deposits
        Thread t1 = new Thread(() -> {
            for (int i = 0; i < 5; i++) alice.deposit(100);
        }, "depositor-1");

        Thread t2 = new Thread(() -> {
            for (int i = 0; i < 5; i++) bob.deposit(50);
        }, "depositor-2");

        t1.start(); t2.start();
        t1.join();  t2.join();

        System.out.println("\nAlice balance: " + alice.getBalance()); // expected 1500
        System.out.println("Bob balance:   " + bob.getBalance());    // expected 750

        // Demo 2: deadlock-safe transfer
        System.out.println();
        Thread transferThread = new Thread(() -> {
            try {
                boolean ok = transfer(alice, bob, 200);
                System.out.println("Transfer result: " + (ok ? "success" : "failed"));
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
            }
        }, "transfer-thread");
        ...
        // Demo 3: interruptible lock
        System.out.println();
        alice.lock.lock(); // Main thread holds the lock
        Thread interruptible = new Thread(() -> {
            try {
                System.out.println("[interruptible-thread] waiting for lock...");
                alice.interruptibleDeposit(50); // will block, then be interrupted
            } catch (InterruptedException e) {
                System.out.println("[interruptible-thread] interrupted while waiting — cancelled");
            }
        }, "interruptible-thread");
        interruptible.start();
        Thread.sleep(100);
        System.out.println("[main] interrupting interruptible-thread");
        interruptible.interrupt();
        interruptible.join();
        alice.lock.unlock(); // Release the lock held by main
    }
}
```
