...
public class ScheduledExecutorDemo {
    ...
    public static void main(String[] args) throws InterruptedException {
        ScheduledExecutorService scheduler = Executors.newScheduledThreadPool(2, r -> {
            Thread t = new Thread(r, "scheduler-thread-" + System.nanoTime() % 1000);
            return t;
        });

        AtomicInteger fixedRateRun  = new AtomicInteger(0);
        AtomicInteger fixedDelayRun = new AtomicInteger(0);

        // ---- scheduleAtFixedRate: metrics collection every 1s ----
        System.out.println("[" + now() + "] Starting scheduleAtFixedRate (period=1s)");
        ScheduledFuture<?> fixedRateTask = scheduler.scheduleAtFixedRate(() -> {
            int run = fixedRateRun.incrementAndGet();
            System.out.printf("[%s] [%s] fixedRate run=%d START%n",
                    now(), Thread.currentThread().getName(), run);
            // Simulate variable task duration: runs 1 and 3 are quick, run 2 is slow
            long sleepMs = (run == 2) ? 1800L : 300L;
            try {
            ...
            System.out.printf("[%s] [%s] fixedRate run=%d END (took %dms)%n",
                    now(), Thread.currentThread().getName(), run, sleepMs);
        }, 0, 1000, TimeUnit.MILLISECONDS);

        // ---- scheduleWithFixedDelay: retry with cool-down of 800ms ----
        System.out.println("[" + now() + "] Starting scheduleWithFixedDelay (delay=800ms)");
        ScheduledFuture<?> fixedDelayTask = scheduler.scheduleWithFixedDelay(() -> {
            int run = fixedDelayRun.incrementAndGet();
            System.out.printf("[%s] [%s] fixedDelay run=%d START%n",
                    now(), Thread.currentThread().getName(), run);
            try {
                Thread.sleep(400); // fixed task duration for clarity
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                return;
            }
            System.out.printf("[%s] [%s] fixedDelay run=%d END%n",
                    now(), Thread.currentThread().getName(), run);
        }, 0, 800, TimeUnit.MILLISECONDS);

        // Let both tasks run for ~4 seconds
        Thread.sleep(4000);

        // Cancel both periodic tasks
        fixedRateTask.cancel(false);
        fixedDelayTask.cancel(false);
        System.out.println("\n[" + now() + "] Both periodic tasks cancelled.");

        // ---- Demonstrate silent failure (bad practice — no try/catch) ----
        System.out.println("\n[" + now() + "] Demo: task without try/catch — future runs silenced");
        ScheduledFuture<?> silentFail = scheduler.scheduleAtFixedRate(() -> {
            System.out.printf("[%s] [%s] silentFail task running%n",
                    now(), Thread.currentThread().getName());
            throw new RuntimeException("simulated uncaught exception");
        }, 0, 500, TimeUnit.MILLISECONDS);

        Thread.sleep(800); // only one execution occurs; rest are silently cancelled
        System.out.printf("[%s] silentFail isDone=%s isCancelled=%s%n",
                now(), silentFail.isDone(), silentFail.isCancelled());
        try {
            silentFail.get();
        } catch (ExecutionException e) {
            System.out.printf("[%s] silentFail.get() threw ExecutionException: %s%n",
                    now(), e.getCause().getMessage());
        } catch (CancellationException e) {
            System.out.printf("[%s] silentFail.get() threw CancellationException%n", now());
        }

        // ---- Fixed version: wrap task body in try/catch ----
        System.out.println("\n[" + now() + "] Demo: task WITH try/catch — executions continue");
        AtomicInteger safeRun = new AtomicInteger(0);
        ScheduledFuture<?> safeTask = scheduler.scheduleAtFixedRate(() -> {
            try {
                int run = safeRun.incrementAndGet();
                System.out.printf("[%s] [%s] safeTask run=%d%n",
                        now(), Thread.currentThread().getName(), run);
                if (run == 2) throw new RuntimeException("expected error on run 2");
            } catch (Exception e) {
                System.out.printf("[%s] safeTask caught exception: %s — next run unaffected%n",
                        now(), e.getMessage());
            }
        }, 0, 500, TimeUnit.MILLISECONDS);

        Thread.sleep(2000);
        safeTask.cancel(false);
        System.out.printf("[%s] safeTask completed %d runs%n", now(), safeRun.get());
