...
/**
 * Demonstrates ExecutorService lifecycle: submit, shutdown, awaitTermination.
 *
 * Run: javac ExecutorLifecycleSnippet.java && java ExecutorLifecycleSnippet
 */
public class ExecutorLifecycleSnippet {

    public static void main(String[] args) throws InterruptedException {
        ExecutorService executor = Executors.newFixedThreadPool(3);

        // Submit a mix of Runnable and Callable tasks
        ...
        executor.shutdown();
        System.out.println("Shutdown initiated. isShutdown=" + executor.isShutdown());

        // Collect results (blocks per future until each task is done)
        for (int i = 0; i < futures.size(); i++) {
            try {
                int result = futures.get(i).get(5, TimeUnit.SECONDS);
                System.out.printf("task-%d result: %d%n", i, result);
            } catch (ExecutionException e) {
                System.out.println("task-" + i + " threw: " + e.getCause());
        ...
        System.out.println("All tasks done. isTerminated=" + terminated);

        // Demonstrate invokeAll
        ExecutorService pool2 = Executors.newFixedThreadPool(2);
        List<Callable<String>> batch = List.of(
                () -> { Thread.sleep(50);  return "result-A"; },
                () -> { Thread.sleep(80);  return "result-B"; },
                () -> { Thread.sleep(30);  return "result-C"; }
        );

        // invokeAll blocks until all tasks are done
        List<Future<String>> batchResults = pool2.invokeAll(batch);
        System.out.println("\n=== invokeAll results ===");
