...
/**
 * Multiple producer threads enqueue work items.
 * Multiple consumer threads poll and process items.
 * Tracks total items produced and consumed.
 * Uses named threads and a CountDownLatch to synchronize startup.
 *
 * Run: javac ConcurrentLinkedQueueDemo.java && java ConcurrentLinkedQueueDemo
 */
public class ConcurrentLinkedQueueDemo {

    static final int PRODUCERS = 3;
    static final int CONSUMERS = 2;
    static final int ITEMS_PER_PRODUCER = 8;
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        // Latch ensures all threads start as close to simultaneously as possible
        ...
        for (int p = 0; p < PRODUCERS; p++) {
            ...
            Thread producer = new Thread(() -> {
                ...
                for (int i = 0; i < ITEMS_PER_PRODUCER; i++) {
                    ...
                    queue.offer(item); // never blocks, never returns false
                    totalProduced.incrementAndGet();
                }
                System.out.printf("[%s] finished producing %d items%n",
                    Thread.currentThread().getName(), ITEMS_PER_PRODUCER);
                producersDone.countDown();
            }, "producer-" + p);
            producer.start();
        }

        // Consumer threads — poll in a loop; stop when producers done and queue empty
        ...
        for (int c = 0; c < CONSUMERS; c++) {
            ...
            consumerThreads[c] = new Thread(() -> {
                ...
                // Spin-poll until producers have finished and queue is drained
                while (true) {
                    ...
                    if (item != null) {
                        ...
                        // Briefly yield to avoid busy-spinning at 100% CPU
                        Thread.yield();
                    }
                }
                System.out.printf("[%s] exiting, consumed so far: %d total%n",
                    Thread.currentThread().getName(), totalConsumed.get());
            }, "consumer-" + consumerId);
        ...
        // Wait for consumers to drain everything
        for (Thread t : consumerThreads) {
            t.join();
        }

        System.out.printf("%nFinal — produced: %d, consumed: %d, remaining: %d%n",
