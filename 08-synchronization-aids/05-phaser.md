...
Four worker threads process data in three phases. Worker-3 deregisters after phase 1. The `onAdvance` override prints phase completion and terminates after phase 2 (the third phase, numbered from 0).
...
public class PhaserMultiPhaseDemo {
    ...
    public static void main(String[] args) throws InterruptedException {
        // Register WORKERS parties + 1 for the main thread
        Phaser phaser = new Phaser(WORKERS + 1) {
            @Override
            protected boolean onAdvance(int phase, int registeredParties) {
                System.out.printf(">>> Phase %d complete. Registered parties now: %d%n",
        ...
        for (int i = 0; i < WORKERS; i++) {
            ...
            workers[i] = new Thread(() -> {
                ...
                // Phase 1 — worker-3 deregisters after this phase
                doWork(name, 1);
                if (id == 3) {
                    System.out.println("[" + name + "] finishing early — deregistering");
                    phaser.arriveAndDeregister();
                    return;
                }
                phaser.arriveAndAwaitAdvance();

                // Phase 2 (workers 1, 2, 4 only)
                doWork(name, 2);
                phaser.arriveAndAwaitAdvance();
        ...
        // Main thread participates in all 3 phases
        for (int phase = 0; phase <= 2; phase++) {
            phaser.arriveAndAwaitAdvance();
            System.out.println("[Main] advanced past phase " + phase
                + " | phaser terminated: " + phaser.isTerminated());
        }

        for (Thread w : workers) w.join();
        System.out.println("Done. Phaser terminated: " + phaser.isTerminated());
    }

    private static void doWork(String name, int phase) {
        try {
            Thread.sleep(100 + RNG.nextInt(200));
            System.out.printf("[%s] completed phase %d work%n", name, phase);
