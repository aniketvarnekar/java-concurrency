...
/**
 * Parallel sum of a long array using RecursiveTask.
 *
...
public class ForkJoinDemo {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < SIZE; i++) {
            array[i] = i + 1L; // values 1 .. 10,000,000
        }

        long expected = (long) SIZE * (SIZE + 1) / 2; // Gauss formula
        System.out.printf("Array size: %,d  |  Expected sum: %,d%n%n", SIZE, expected);
        ...
        // --- Parallel Fork/Join sum (custom pool, parallelism=4) ---
        ForkJoinPool pool = new ForkJoinPool(4);
        try {
            long fjStart = System.currentTimeMillis();
            long fjSum = pool.invoke(new ParallelSum(array, 0, SIZE));
            long fjTime = System.currentTimeMillis() - fjStart;
            System.out.printf("ForkJoin  sum  : %,d  in %d ms  (parallelism=4)%n",
                    fjSum, fjTime);

            System.out.printf("%nResults match: %b%n", seqSum == fjSum && fjSum == expected);
        } finally {
