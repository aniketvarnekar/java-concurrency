...
/**
 * Three parallel stream scenarios: correct sum, broken shared-state accumulation, and the fix.
 *
 * Run: javac ParallelStreamsDemo.java && java ParallelStreamsDemo
 */
public class ParallelStreamsDemo {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // ---------------------------------------------------------------
        // Scenario 1: Correct parallel sum using reduce/sum
        // ---------------------------------------------------------------
        System.out.println("=== Scenario 1: Correct parallel sum ===");
        ...
        System.out.printf("  Sequential sum: %,d in %d ms%n", seqSum, seqTime);
        System.out.printf("  Parallel   sum: %,d in %d ms%n", parSum, parTime);
        System.out.printf("  Results correct: %b%n%n", seqSum == expectedSum && parSum == expectedSum);

        // ---------------------------------------------------------------
        // Scenario 2: BROKEN — shared ArrayList, data race
        // ---------------------------------------------------------------
        System.out.println("=== Scenario 2: Broken — shared ArrayList (data race) ===");
        List<Integer> brokenResult = new ArrayList<>();
        // This is intentionally broken to demonstrate the race:
        numbers.parallelStream().limit(10_000).forEach(brokenResult::add);
        System.out.printf("  Expected 10,000 elements; got: %,d  (data race: elements dropped)%n%n",
                brokenResult.size());

        // ---------------------------------------------------------------
        // Scenario 3: Fixed — use collect() for thread-safe aggregation
        // ---------------------------------------------------------------
        System.out.println("=== Scenario 3: Fixed — collect() is thread-safe ===");
        List<Integer> fixedResult = numbers.parallelStream()
                .limit(10_000)
                .collect(Collectors.toList());
        System.out.printf("  Expected 10,000 elements; got: %,d  (correct)%n%n",
                fixedResult.size());

        // ---------------------------------------------------------------
        // Show parallelism: print thread names from a parallel stream
        // ---------------------------------------------------------------
        System.out.println("=== Threads used by parallel stream (first 8 elements) ===");
        Set<String> threadNames = Collections.synchronizedSet(new LinkedHashSet<>());
        numbers.parallelStream()
                .limit(50_000)
                .forEach(x -> threadNames.add(Thread.currentThread().getName()));
        System.out.println("  Threads involved: " + threadNames);
    }
}
```
...
**`sorted()` on a parallel stream buffers all elements before sorting.** The stream must collect all elements from parallel threads, sort them (using `Arrays.parallelSort()` internally, which is itself parallel), and then emit them in order to downstream operations. For large datasets, this can be faster than sequential sort. For typical sizes, it adds memory allocation and synchronization overhead that rarely pays off. If ordering is not needed after sorting (e.g., the final result is a set), call `.unordered()` before `.sorted()` to allow optimizations.
