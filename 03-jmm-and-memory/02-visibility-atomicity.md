...
This example demonstrates `volatile` used correctly for a stop flag, then shows why `volatile` on a counter is still broken for concurrent increments.
...
public class VisibilityVsAtomicity {
    ...
    // Correct use of volatile: single-writer flag for visibility
    private static volatile boolean stopFlag = false;

    // Broken: volatile gives visibility but not atomicity for read-modify-write
    private static volatile int volatileCounter = 0;

    // Correct: AtomicInteger gives both visibility and atomicity
    private static final AtomicInteger atomicCounter = new AtomicInteger(0);

    public static void main(String[] args) throws InterruptedException {
        demonstrateVolatileFlag();
        demonstrateAtomicityGap();
    }

    static void demonstrateVolatileFlag() throws InterruptedException {
        stopFlag = false;

        Thread worker = new Thread(() -> {
            System.out.println("[flag-worker] starting loop");
            long iters = 0;
            while (!stopFlag) {
                iters++;
            }
            System.out.println("[flag-worker] stopped after " + iters + " iterations");
        }, "flag-worker");

        worker.start();
        Thread.sleep(50);
        System.out.println("[main] setting stopFlag = true");
        stopFlag = true;
        worker.join();
        System.out.println("[main] flag-worker terminated cleanly");
    }

    static void demonstrateAtomicityGap() throws InterruptedException {
        volatileCounter = 0;

        // Ten threads, each incrementing 1000 times
        ...
        System.out.println("[main] expected counter value: 10000");
        System.out.println("[main] volatile counter (racy): " + volatileCounter);
        System.out.println("[main] atomic counter (correct): " + atomicCounter.get());
    }
}
```

Running this program will typically show the volatile counter at a value less than 10000, demonstrating lost updates. The atomic counter will always show 10000.
